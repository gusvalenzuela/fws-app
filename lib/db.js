import { createMatchupDocToUpdate, convertToSportId } from './api-helpers'
...
// USER PICKS
export async function getUserPicks({ db }, id) {
...
export async function updateSingleMatchup(db, data) {
  ...
  // clean with helper function to remove unused fields
  const documentToUpdate = createMatchupDocToUpdate({ ...filter, ...data })
